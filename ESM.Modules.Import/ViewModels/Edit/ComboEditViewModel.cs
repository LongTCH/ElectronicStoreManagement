...
{
    public class ComboEditViewModel : BindableBase, IViewModel, INavigationAware
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IModalService _modalService;
        private readonly ViewModelStore _viewModelStore;

        public ComboEditViewModel(IUnitOfWork unitOfWork, IModalService modalService, ViewModelStore viewModelStore)
        {
        ...
        public async void OnNavigatedTo(NavigationContext navigationContext)
        {
            ...
            Id = navigationContext.Parameters["comboId"].ToString();
            var p = await _unitOfWork.Combos.GetById(Id);
            Name = p.Name;
            Unit = p.Unit;
            Discount = p.Discount;
            var list = await _unitOfWork.Combos.GetListProduct(p);
        ...
        public async Task save()
        {
            ...
            if (metroWindow.ShowModalMessageExternal("Thông báo", "Bạn có chắc chắn lưu?", MessageDialogStyle.AffirmativeAndNegative) == MessageDialogResult.Affirmative)
            {
                ...
                bool res = (bool)await _unitOfWork.Combos.Update(p);
                if (res)
                {
