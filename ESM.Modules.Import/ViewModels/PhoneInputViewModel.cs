...
namespace ESM.Modules.Import.ViewModels
{
    public class PhoneInputViewModel : BaseProductInputViewModel<Smartphone>
    {
        ...
        protected override async void addCommand()
        {
            ...
            {
                if (Id == null || Id.Length != 9 || !Id.StartsWith(DAStaticData.IdPrefix[ProductType.SMARTPHONE]) || !Id.All(x => char.IsDigit(x)))
                {
                ...
                var task2 = _unitOfWork.Smartphones.IsIdExist(Id);
                ...
                Smartphone smartphoneDTO = GetPc();
                ProductList.Add(smartphoneDTO);
            ...
            else if (SelectedWorkType == "SỬA")
            {
                ...
                Smartphone smartphoneDTO = GetPc();
                smartphoneDTO.Remain = Remain;
                var res = await _unitOfWork.Smartphones.Update(smartphoneDTO);
                if ((bool)res)
                ...
                // Clear
                Empty();
                var list = await _unitOfWork.Smartphones.GetAll();
        ...
        protected override async void deleteCommand(ProductDTO productDTO)
        {
            ...
            else
            {
                ...
                {
                    await _unitOfWork.Smartphones.Delete(productDTO.Id);
                    var list = await _unitOfWork.Smartphones.GetAll();
                    ProductList = new(list);
                }
            }
        ...
        {
            var res = await _unitOfWork.Smartphones.AddList(ProductList);
            if ((bool)res)
            {
        ...
        protected override void editCommand(ProductDTO productDTO)
        {
            if (SelectedWorkType == "THÊM")
            {
                ProductList.Remove((Smartphone)productDTO);
                NotInDatabase.Remove(productDTO.Id);
            }
            findCommand(productDTO);
        }
        private Smartphone GetPc()
        {
            return new()
            {
                Id = Id,
